...
def data_save_model():
    ...
    table_chart_link = table_chart_link['Link'].values
    ...
    for data in range(len(table_Symbole_yesterday)):
        ...
        table_symbole2.table_today_open= int(saved_data[data])#float(table_today_open[data].replace(',', ''))
        table_symbole2.table_extra =table_Symbole_today[data]
        table_symbole2.table_chart = table_chart_link[data]
        table_symbole2.save()
    print("maintabledone")
