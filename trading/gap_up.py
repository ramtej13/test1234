...
def data_save_model_gap_up():
    ...
    try:
        ...
        for data in range(len(difference)):
            high = float(table_high_yesterday[data].replace(',',''))
            open = float(table_today_open[data].replace(',',''))
            diff = ((open-high)/high)*100
        ...
        #     Gap_up.objects.filter(gap_up_yesterday_symbol=data).delete()

        #new method
        datadelete = Gap_up.objects.all().delete()

        for data in range(len(table_Symbole_yesterday)):
            table_symbole_gap_up = Gap_up()
            table_symbole_gap_up.gap_up_current_price= float(table_ltp_today[data].replace(',', ''))
            table_symbole_gap_up.gap_up_open = float(table_today_open[data].replace(',', ''))
            table_symbole_gap_up.gap_up_percentage = int(saved_data[data])
            table_symbole_gap_up.gap_up_yesterday_high = float(table_high_yesterday[data].replace(',', ''))
            table_symbole_gap_up.gap_up_yesterday_symbol= table_Symbole_yesterday[data]
            table_symbole_gap_up.gap_up_today_symbol = table_Symbole_today[data]
            table_symbole_gap_up.gap_up_chat_zerodha = table_chart_zerodha[data]
            table_symbole_gap_up.gap_up_chat_marketmojo = table_chart_marketmojo[data]
            table_symbole_gap_up.gap_up_chat_economictimes = table_chart_etnow[data]
            table_symbole_gap_up.save()
        return True
