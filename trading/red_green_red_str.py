...
def data_save_model_red_green_red():
    ...
    try:
        ...
        # data_extract_yesterday = data_extract_yesterday.drop(['LOW \n', 'PREV. CLOSE \n',
        #                                                       'CHNG \n', 'VOLUME \n(shares)', 'VALUE ', '52W H \n',
        #                                                       '52W L \n', '365 D % CHNG \n 03-May-2019',
        #                                                       '30 D % CHNG \n 03-Apr-2020'] ,axis=1)
        ...
        table_change_yesterday = data_extract_yesterday['%CHNG \n'].values
        table_high_yesterday = data_extract_yesterday['HIGH \n'].values
        table_open_yesterday = data_extract_yesterday['OPEN \n'].values
        ...
        for data in range(len(difference)):
            close = float(table_close_yesterday[data].replace(',',''))
            open = float(table_today_open[data].replace(',',''))
            diff = close-open
            newpercentage1 = table_change_yesterday[data]+"0"
        ...
        #     Red_green_red.objects.filter(red_green_red_yesterday_symbol=data).delete()

        #newmethod
        datadelete = Red_green_red.objects.all().delete()

        #saving the new data
        for data in range(len(table_Symbole_yesterday)):

            table_symbole_red = Red_green_red()
            table_symbole_red.red_green_red_current_price= float(table_close_yesterday[data].replace(',', ''))
            table_symbole_red.red_green_red_open = float(table_today_open[data].replace(',', ''))
            print([data,table_Symbole_yesterday[data]])
            table_symbole_red.red_green_red_percentage_change = float(newpercentage[data])#.replace(',', ''))
            table_symbole_red.red_green_red_difference = int(saved_data[data])
            table_symbole_red.red_green_red_yesterday_high = float(table_high_yesterday[data].replace(',', ''))
            table_symbole_red.red_green_red_yesterday_symbol =table_Symbole_yesterday[data]
            table_symbole_red.red_green_red_today_symbol = table_Symbole_today[data]
            table_symbole_red.red_green_red_chat_zerodha = table_chart_zerodha[data]
            table_symbole_red.red_green_red_chat_marketmojo = table_chart_marketmojo[data]
            table_symbole_red.red_green_red_chat_economictimes = table_chart_etnow[data]
            table_symbole_red.save()
        return True
